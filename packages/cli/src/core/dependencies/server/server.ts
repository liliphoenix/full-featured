...
import { PackageAnalyzer } from '../Analyzer/analyzer'
...
export function createDataServer(analyzer: PackageAnalyzer, ui: string) {
  ...
  portfinder
    ...
    .then((port) => {
      app.listen(port, () => {
        console.log(
          `  ${chalk.green('->')}  ${chalk.gray('Local: ')}   ${chalk.blue.underline(`http://localhost:${port}`)}`
        )
        console.log(
          `  ${chalk.red('->')}  press Ctrl+C to terminate the process`
        )
